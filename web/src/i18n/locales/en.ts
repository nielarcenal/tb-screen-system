/**
 * English strings — source of truth for keys (brief §9: no hardcoded UI text).
 * POSITIONING (§1): the portal is where TB-DOTS staff RECORD laboratory results;
 * screening data shown here is pre-screening information, never a diagnosis
 * made by the system. Copy must never imply the system detects TB.
 */
export const en = {
  common: {
    loading: 'Loading…',
    refresh: 'Refresh',
    signOut: 'Sign out',
    yes: 'Yes',
    no: 'No',
    unsure: 'Unsure',
    back: '← Back to inbox',
    all: 'All',
    nonDiagnostic:
      'Screening answers shown here are pre-screening information gathered by a BHW — not a diagnosis. Diagnosis happens at the TB-DOTS facility through laboratory testing.',
  },
  /** D-12: the portal's terminal account states. Shown by AccountStateGate
   *  instead of the portal, each with a way back to the sign-in screen. */
  account: {
    errorTitle: 'Could not load your account',
    errorBody:
      'You are signed in, but your account details could not be reached. Check your connection and try again.',
    missingTitle: 'This account is not set up yet',
    missingBody:
      'Your sign-in worked, but no portal account is linked to it yet. Ask your administrator to finish setting it up, then sign in again.',
    inactiveTitle: 'This account has been deactivated',
    inactiveBody:
      'Access to the portal has been turned off for this account. Contact your administrator if you think this is a mistake.',
  },
  languages: {
    /** Accessible group name for the EN/TL/CEB switcher. The pills themselves
     *  read 'EN'/'TL'/'CEB', so this label is the only thing a screen reader
     *  announces for the control. */
    label: 'Language',
  },
  nav: {
    dashboard: 'Dashboard',
    bhws: 'BHW management',
    inbox: 'Referral inbox',
    cases: 'TB case registry',
    register: 'Register patient',
    hotspot: 'Barangay hotspots',
    report: 'Barangay report',
    bhw: 'BHW management',
    midwives: 'Midwife management',
    staff: 'Staff management',
    audit: 'Activity log',
  },
  audit: {
    title: 'Facility activity log',
    privacy: 'Who changed what, and when. No clinical notes, lab-result values or contact details are stored here.',
    filterLabel: 'Filter by record type',
    empty: 'No recorded activity yet.',
    end: 'That is the whole log.',
    loadMore: 'Load older activity',
    loadError: 'The activity log could not be loaded.',
    noFields: 'No field changes recorded.',
    changed: 'changed',
    notSet: 'not set',
    yes: 'yes',
    no: 'no',
    byName: '{{name}} ({{role}})',
    byRole: 'A {{role}} account',
    bySystem: 'System or support action',
    filter: { all: 'All' },
    entity: {
      appointments: 'Check-up',
      tb_cases: 'TB case',
      treatment_followups: 'Treatment visit',
      referrals: 'Referral',
      case_lab_results: 'Case lab result',
      case_vitals: 'Case vital signs',
    },
    action: {
      created: 'Created',
      updated: 'Updated',
      status_changed: 'Status changed',
      transferred: 'Transferred',
      cancelled: 'Cancelled',
      closed: 'Closed',
      voided: 'Voided',
    },
    field: {
      status: 'Status',
      scheduled_date: 'Scheduled date',
      attended_date: 'Attended date',
      facility_id: 'Facility',
      referral_id: 'Linked referral',
      tb_case_id: 'Linked case',
      case_status: 'Case status',
      treatment_start_date: 'Treatment start',
      outcome: 'Outcome',
      outcome_date: 'Outcome date',
      registration_date: 'Registration date',
      case_number: 'Case number',
      visit_date: 'Visit date',
      voided_at: 'Voided',
      void_reason: 'Void reason',
      presented: 'Presented',
      result_date: 'Result date',
      lab_sample_id: 'Lab sample',
      measured_on: 'Measured on',
    },
  },
  shell: {
    facilityPortal: 'Facility Portal',
    bhwsSub: 'Manage BHW accounts across every barangay',
    midwifePortal: 'Midwife Portal',
    dashboardSub: 'Today’s activity at your facility',
    referralsSub: 'Referrals addressed to your facility',
    casesSub: 'Treatment episodes managed by your facility',
    registerSub: 'Walk-in and self-referred patients',
    hotspotsSub: 'Presumptive cases by barangay',
    reportSub: 'Screening and referral counts per barangay',
    bhwSub: 'Health workers in your barangay',
    auditSub: 'Record changes made at this facility',
    adminPortal: 'Developer Portal',
    roleAdmin: 'Administrator',
    midwivesSub: 'Provision Barangay Midwife accounts',
    staffSub: 'Provision TB-DOTS staff accounts',
    midwifeDashSub: 'Your BHW team at a glance',
    facilitiesSub: 'Manage TB-DOTS facilities',
  },
  capDash: {
    summaryTitle: 'This month',
    bhws: 'BHWs',
    screenings: 'Screenings',
    referrals: 'Referrals',
    last30: 'last 30 days',
    activeLc: 'active',
    inactiveLc: 'inactive',
    activityTitle: 'Activity this month',
    activitySub: 'Screenings and referrals per BHW, last 30 days',
    wordScreenings: 'screenings',
    wordReferrals: 'referrals',
    emptyTitle: 'No BHWs yet',
    emptyBody: 'Once you add BHWs, their monthly activity appears here.',
    errorTitle: 'Couldn’t load your dashboard',
    errorBody: 'We couldn’t reach the server. Check your connection and try again.',
    retry: 'Retry',
    footerNote:
      'Activity counts are totals only — no patient information is shown to midwives.',
  },
  adminDash: {
    headerSub: 'Program-wide overview',
    deactivated: '{{count}} deactivated',
    summaryTitle: 'Program overview',
    facilities: 'Facilities',
    facilitiesSub: 'TB-DOTS centers',
    midwives: 'Midwives',
    midwivesSub: 'across the program',
    bhws: 'BHWs',
    bhwsSub: 'across all facilities',
    openReferrals: 'Open referrals',
    openSub: 'system-wide',
    coverageTitle: 'Facility coverage',
    coverageSub: 'Midwives, BHWs and open referrals per facility',
    covMidwives: 'midwives',
    covBhws: 'BHWs',
    covOpen: 'open',
    emptyTitle: 'No facilities yet',
    emptyBody: 'TB-DOTS facilities will appear here once they exist.',
    errorTitle: 'Couldn’t load the overview',
    errorBody: 'We couldn’t reach the server. Check your connection and try again.',
    retry: 'Retry',
  },
  facilities: {
    title: 'Facilities',
    addCta: 'New facility',
    searchPlaceholder: 'Search facility or address',
    nameLabel: 'Facility name',
    addressLabel: 'Address',
    addTitle: 'New facility',
    editTitle: 'Edit facility',
    create: 'Create facility',
    typeChip: 'TB-DOTS',
    empty: 'No facilities yet.',
    emptyDataBody: 'Add a TB-DOTS facility to get started.',
    filterEmptyTitle: 'No facilities match',
    errorTitle: 'Couldn’t load facilities',
    saveError: 'Could not save the facility: {{message}}',
    noAddress: 'No address on file',
  },
  admin: {
    sub: 'Account provisioning — developer access only',
    notAdmin: 'This account is not a developer account.',
    goMain: 'Go to the facility portal',
    goMidwife: 'Go to the midwife portal',
    footnote:
      'Provisioning only: no patient names, records, or results are accessible from this portal.',
  },
  staff: {
    title: 'TB-DOTS staff accounts',
    addCta: 'Add staff',
    addTitle: 'Add TB-DOTS staff',
    editTitle: 'Edit staff',
    facilityLabel: 'Facility',
    filterLabel: 'Facility',
    createdTitle: 'Staff account created',
    scopeNote:
      'The account only sees referrals addressed to the facility chosen here — it defines their whole portal.',
    empty: 'No staff accounts yet.',
    errorTitle: 'Couldn’t load staff',
    filterEmptyTitle: 'No staff match',
    emptyDataBody: 'Add a staff account to get started.',
    filterEmptyBody: 'Try clearing the search or facility filter.',
  },
  midwives: {
    title: 'Barangay Midwife accounts',
    privacyNote:
      'Admins provision midwife accounts only. No patient names, records, or results appear anywhere in this view.',
    addCta: 'Add midwife',
    addTitle: 'Add Barangay Midwife',
    editTitle: 'Edit midwife',
    facilityLabel: 'Facility (nearest TB-DOTS center)',
    noFacilityMapped: 'no DOTS center mapped for this barangay — apply migration 0009',
    createdTitle: 'Midwife account created',
    scopeNote:
      'The midwife will only be able to add and manage BHW accounts for the barangay assigned here.',
    empty: 'No midwife accounts yet.',
    errorTitle: 'Couldn’t load midwives',
    filterEmptyTitle: 'No midwives match',
    emptyDataBody: 'Add a midwife to get started.',
  },
  dashboard: {
    outcomeNote:
      'Result outcomes (positive / negative) are recorded and visible only at the facility. BHWs see referral progress — submitted, received, tested, result recorded — never the outcome itself.',
    screened: 'Screened today',
    screenedSub: 'Screenings linked to this facility today',
    referred: 'New referrals',
    referredSub: 'Addressed to this facility today',
    positive: 'Tested positive',
    positiveSub: 'Outcomes recorded by this facility today',
    negative: 'Tested negative',
    negativeSub: 'Outcomes recorded by this facility today',
    attended: 'Attended check-up',
    attendedSub: 'Of {{total}} scheduled today',
    missed: 'Missed check-up',
    missedSub: 'BHW follow-up lists updated',
    cardTitle: 'Today’s activity',
    errorTitle: 'Couldn’t load the dashboard',
    errorBody: 'We couldn’t reach the server. Check your connection and try again.',
    retry: 'Retry',
    updatedNote: 'Counts are current when refreshed.',
    attention: {
      title: 'Attention required',
      note: 'These are operational facts, not a patient risk score. Overdue means the date passed while the appointment remains open; missed means staff recorded a no-show.',
      overdue: 'Cases with overdue follow-ups',
      overdueSub: 'Past date and still open',
      missed: 'Cases with unresolved missed follow-ups',
      missedSub: 'No later visit is scheduled or attended',
      dueSoon: 'Cases with follow-ups due soon',
      dueSoonSub: 'Tomorrow through the next 7 days',
      awaiting: 'Referrals awaiting action',
      awaitingSub: 'Submitted or received',
      stale: 'Cases without a recent visit',
      staleSub: 'Active for 31+ days; no visit in 30 days',
      today: 'Treatment appointments today',
      todaySub: 'Open appointments on active cases',
    },
    program: {
      title: 'Program overview',
      note: 'Current facility-scoped operational totals. Screened counts distinct screenings attached to this facility’s referrals.',
      screened: 'Screened',
      screenedSub: 'Linked to this facility’s referrals',
      referred: 'Referred',
      received: 'Referral received',
      receivedSub: 'Currently received, tested, or closed',
      cases: 'Cases created',
      active: 'Active treatment cases',
      activeSub: 'On treatment or interrupted',
      due: 'Follow-ups due',
      dueSub: 'Open and due through today',
      missed: 'Missed follow-ups',
      missedSub: 'Recorded and not yet followed by a visit',
      closed: 'Completed / closed cases',
      allTime: 'All recorded time',
    },
  },
  bhw: {
    title: 'BHW accounts',
    barangayHint: 'The barangay this BHW will serve. Sets their facility automatically.',
    barangayLabel: 'Barangay',
    barangayAll: 'All barangays',
    barangayNone: 'No barangay assigned',
    groupCount_one: '{{count}} worker',
    groupCount_other: '{{count}} workers',
    privacyNote: 'This view shows accounts and activity counts only. No patient names, records, or results appear anywhere in it.',
    addCta: 'New BHW',
    colStatus: 'Status',
    active: 'Active',
    deactivated: 'Deactivated',
    edit: 'Edit',
    deactivate: 'Deactivate',
    reactivate: 'Reactivate',
    editTitle: 'Edit BHW',
    firstNameLabel: 'First name',
    lastNameLabel: 'Last name',
    cancel: 'Cancel',
    create: 'Create account',
    save: 'Save changes',
    createdTitle: 'BHW account created',
    resetPw: 'Reset password',
    resetDoneTitle: 'Password reset',
    emailLabel: 'Email',
    tempPwLabel: 'Temporary password',
    createdNote:
      'Share these with the BHW in person. They sign in with the temporary credential and should change their password afterwards.',
    done: 'Done',
    confirmDeactivate:
      'Deactivate {{name}}? They will no longer be able to sign in or submit screenings. Past records stay intact. You can reactivate this account anytime.',
    deactivateTitle: 'Deactivate this BHW?',
    activityLabel: 'Activity in the last 30 days',
    reassignLabel: 'Reassign coverage to',
    reassignNone: 'Leave unassigned for now',
    empty: 'No BHW accounts in your facility yet.',
    actionError: 'Action failed: {{message}}',
    searchPlaceholder: 'Search name',
    errorTitle: 'Couldn’t load BHWs',
    errorBody: 'We couldn’t reach the server. Try again.',
    retry: 'Retry',
    emptyDataBody: 'Add a BHW to start building your team.',
    filterEmptyTitle: 'No BHWs match',
    filterEmptyBody: 'Try clearing the search or status filter.',
    resultsCount_one: '{{count}} result',
    resultsCount_other: '{{count}} results',
    newTitle: 'New BHW',
    newSub: 'Create a BHW account in {{barangay}}',
    middleNameLabel: 'Middle name',
    optional: 'optional',
    coverageLabel: 'Coverage area',
    coveragePlaceholder: 'e.g. Purok 3',
    emailAuto: 'Generated automatically from the name.',
    emailPending: 'Enter first and last name',
    createBhw: 'Create BHW',
    savedFlash: 'Saved',
    statusAll: 'All statuses',
    ctxCoverage: 'Coverage area',
    ctxScreenings: 'Screenings',
    ctxReferrals: 'Referrals',
    ctxJoined: 'Joined',
    reqNote: 'First name, last name and coverage area required.',
    reqNoteAdmin: 'First name, last name, barangay and coverage area required.',
    hoWhat: 'What happens',
    hoPoint1: 'Loses app access immediately.',
    hoPoint2: 'Their screening and referral history is retained and stays with the facility.',
    hoPoint3: 'Their coverage area needs another BHW — reassign it below, or leave it for now.',
    hoConfirm: 'Deactivate BHW',
  },
  report: {
    cityFilter: 'City / municipality (view and download)',
    downloadPdf: 'Download PDF',
    pdfError: 'PDF download failed. Please try again or download the printable HTML.',
    demoDisclaimer: 'CAPSTONE DEMO ONLY — All data in this report is fictional and does not represent real patients or actual health statistics. This is not an official DOH/ITIS report and must not be used for clinical or public-health decisions.',
    downloadPrintable: 'Download printable report',
    printInstruction: 'To print or save as PDF, use your browser’s Print command (Ctrl+P). Select A4 landscape. This file works offline.',
    generatedOn: 'Generated on',
    exportScope: 'Downloads include every barangay in the selected city/municipality (or all Bukidnon), including zero-record barangays, for both periods. Search and pagination do not limit downloads.',
    periodFull: 'Showing all of {{year}}, compared with all of the year before.',
    periodPartial:
      'Showing 1 January to {{end}}. The year before covers the SAME dates, so the two columns can be compared fairly.',
    mScreened: 'Screened',
    mReferred: 'Referred',
    mCases: 'Registered cases',
    mSuccessful: 'Cured / completed',
    mLtfu: 'Lost to follow-up',
    dScreened: 'Checked for TB symptoms',
    dReferred: 'Sent to TB-DOTS for testing',
    dCases: 'TB cases registered for treatment',
    dSuccessful: 'Successful outcomes recorded by staff',
    dLtfu: 'Lost-to-follow-up outcomes recorded by staff',
    nReferred: 'referrals',
    nCases: 'registered TB cases',
    nSuccessful: 'successful outcomes',
    nLtfu: 'lost-to-follow-up outcomes',
    nScreened: 'people screened',
    rankedTitle: 'Top barangays by {{metric}}',
    tableTitle: 'All barangays',
    colBarangay: 'Barangay',
    colCity: 'City / municipality',
    viewAll: 'View all barangays',
    searchPlaceholder: 'Search barangay…',
    showing: 'Showing {{from}}–{{to}} of {{total}} barangays',
    showingNone: 'No barangays to show',
    noMatch: 'No barangay matches “{{q}}”.',
    prevPage: 'Previous page',
    nextPage: 'Next page',
    yearGroup: 'Year',
    metricGroup: 'Rank by',
    cityLabel: 'Bukidnon province',
    cityNote: 'Rows are grouped by the patient’s home barangay across Bukidnon.',
    defCases: 'cases registered by a TB-DOTS facility in the selected year.',
    defSuccessful: 'cases recorded as cured or treatment completed, counted in the outcome year.',
    defLtfu: 'cases explicitly recorded by staff as lost to follow-up, counted in the outcome year.',
    ltfuCaveat: 'Lost to follow-up is a recorded clinical program outcome. A late or missed appointment is not automatically classified as lost to follow-up.',
    exportCsv: 'Download CSV',
    empty: 'Nothing recorded for this measure.',
    emptyBody: 'No barangay has a count above zero. Try another measure, or another year.',
    errorTitle: 'Couldn’t load the report',
    errorBody: 'We couldn’t reach the server. Check your connection and try again.',
    retry: 'Retry',
    scopeNoteV2: 'Counts only records entered in TB-Screen. This supports the health office register but does not replace DOH ITIS or clinical judgment.',
  },
  hotspot: {
    intro:
      'Presumptive case counts by barangay — patients flagged for referral by the DOH-NTP checklist in the chosen period, counted once each, across all BHWs. Counts only; surveillance summary, not contact tracing.',
    rangeLast: 'Last {{days}} days',
    countsNote: 'Counts are presumptive referrals only — not confirmed cases, not diagnoses.',
    empty: 'No presumptive cases in this period.',
    errorTitle: 'Couldn’t load hotspots',
    errorBody: 'We couldn’t reach the server. Check your connection and try again.',
    emptyBody: 'No barangay has been flagged for referral in this period. Try a longer range.',
    unit: 'cases',
    retry: 'Retry',
  },
  login: {
    roleStaff: 'TB-DOTS staff',
    roleMidwife: 'Barangay Midwife',
    email: 'Email',
    password: 'Password',
    showPassword: 'Show password',
    hidePassword: 'Hide password',
    cta: 'Sign in',
    error: 'Sign-in failed: {{message}}',
    brandTag: 'Health Worker Platform',
    brandHead: 'Community TB screening, coordinated in one place.',
    brandSub:
      'One platform connecting barangay health workers, midwives, and TB-DOTS facilities across Bukidnon.',
    pSecure: 'Secure, role-based access',
    pRoles: 'For BHWs, midwives & facilities',
    pProgram: 'Bukidnon TB-DOTS Program',
    copyright: 'TB-Screen · Bukidnon TB-DOTS',
    signinTitle: 'Sign in',
    accessHelp: 'Contact your administrator if you can’t sign in.',
    noteStaff:
      'TB-DOTS staff receive referrals, record laboratory results, and track check-up attendance.',
    noteMidwife:
      'Barangay Midwives manage the health workers assigned to their own barangay.',
    goFacility: 'TB-DOTS staff sign in here',
    goMidwife: 'Barangay Midwives sign in here',
  },
  /**
   * Forced password change (D-06). Shown while the signed-in account still
   * holds the password manage-bhw generated for it.
   */
  password: {
    gateTitle: 'Set your own password',
    gateSub:
      'This account is still using the password your administrator gave you. Choose a password only you know to continue.',
    signedInAs: 'Signed in as {{email}}',
    newLabel: 'New password',
    confirmLabel: 'Confirm new password',
    hint: 'At least {{min}} characters. Do not reuse the password you were given.',
    cta: 'Save password and continue',
    saving: 'Saving…',
    signOut: 'Sign out instead',
    errTooShort: 'Use at least {{min}} characters.',
    errLooksProvisioned: 'That is a password the system generated. Choose one of your own.',
    errMismatch: 'The two passwords do not match.',
    errApi: 'Could not save the password: {{message}}',
    errFlag:
      'Your password was changed, but this browser could not finish. Sign in again using your new password.',
  },
  inbox: {
    repeatReferral: 'Repeat',
    repeatReferralHint: 'Referral {{ordinal}} for this patient at your facility.',
    title: 'Referral inbox',
    empty: 'No referrals for your facility yet.',
    searchPlaceholder: 'Search patient or specimen code…',
    statusFilter: 'Status',
    awaitingAction: 'Awaiting facility action',
    presentedNo: 'No-show',
    count_one: '{{count}} referral',
    count_other: '{{count}} referrals',
    filterEmptyTitle: 'No referrals match',
    filterEmptyBody: 'Try clearing the search or status filter.',
    errorTitle: 'Couldn’t load referrals',
    errorBody: 'We couldn’t reach the server. Check your connection and try again.',
    retry: 'Retry',
  },
  status: {
    submitted: 'Submitted',
    received: 'Received',
    tested: 'Tested',
    closed: 'Closed',
  },
  sex: {
    male: 'Male',
    female: 'Female',
  },
  cases: {
    sectionsLabel: 'Case record sections',
    sections: { overview: 'Screening & referral', treatment: 'Treatment & visits', results: 'Lab & vital signs', history: 'Patient history' },
    recordDetails: 'Record identifiers & audit details',
    chooseOutcome: 'Select a treatment outcome',
    returnNotice: 'Returning after lost to follow-up? Keep this episode closed. A clinician must reassess the patient before a new treatment episode is registered under the same patient record. Do not create a duplicate patient.',
    detailTitle: 'TB case details',
    caseNumber: 'Case number',
    unknownPatient: 'Patient record unavailable',
    patientCode: 'Patient code',
    registrationDate: 'Registration date',
    treatmentStart: 'Treatment start',
    owningFacility: 'Owning facility',
    originReferral: 'Originating referral',
    walkIn: 'Facility walk-in',
    createdAt: 'Created',
    createdBy: 'Recorded by user',
    outcomeLabel: 'Treatment outcome',
    startDate: 'Treatment start date',
    startTreatment: 'Start treatment',
    cancelCase: 'Cancel case opened in error',
    markInterrupted: 'Mark treatment interrupted',
    orCloseBelow: 'Or record a treatment outcome below.',
    resumeTreatment: 'Resume treatment',
    closeCase: 'Close case',
    outcomeDate: 'Outcome date',
    actionError: 'The case was not changed.',
    followups: 'Recorded visits',
    noFollowups: 'No visits recorded yet.',
    voided: 'Voided record',
    noNotes: 'No notes recorded.',
    appointments: 'Case appointments',
    noAppointments: 'No case appointments yet.',
    preScreening: 'Pre-screening & PGI-S',
    noScreening: 'No originating pre-screening is linked to this case.',
    labReport: 'Diagnostic result (referral)',
    noLabReport: 'No originating laboratory referral is linked to this case.',
    labOutcome: 'Recorded outcome',
    labDate: 'Result date',
    labNotes: 'Result notes',
    search: 'Search patient or case number…',
    filter: 'Case filter',
    loadError: 'Could not load the case registry',
    empty: 'No cases match this view.',
    selectCase: 'Select a case to view its treatment record.',
    latestVisit: 'Latest visit',
    nextAppointment: 'Next appointment',
    status: {
      registered: 'Registered',
      onTreatment: 'On treatment',
      interrupted: 'Interrupted',
      closed: 'Closed',
      cancelled: 'Cancelled',
    },
    outcome: {
      cured: 'Cured',
      completed: 'Treatment completed',
      failed: 'Treatment failed',
      died: 'Died',
      lost: 'Lost to follow-up',
      notEvaluated: 'Not evaluated',
    },
    filterValue: {
      all: 'All cases',
      active: 'Active',
      closed: 'Closed',
      followup_due: 'Follow-up due',
      overdue: 'Overdue follow-up',
      due_soon: 'Due in the next 7 days',
      appointments_today: 'Appointments today',
      stale: 'No recent visit',
      missed: 'Missed follow-up',
    },
    attention: {
      interrupted: 'Treatment interrupted',
      missed: 'Missed check-up',
      due: 'Follow-up due',
    },
    visit: {
      title: 'Record treatment visit',
      terminal: 'This case has ended and cannot accept new visits.',
      appointment: 'Appointment attended',
      unscheduled: 'Unscheduled visit',
      date: 'Visit date',
      notes: 'Clinical notes (optional)',
      notesPlaceholder: 'Record only what is needed for this treatment visit.',
      statusAfter: 'Case status after visit',
      noStatusChange: 'No status change',
      nextDate: 'Next follow-up date (optional)',
      save: 'Record visit',
      saving: 'Recording visit…',
      corrections: 'Visit corrections',
      correctDate: 'Correct date',
      correctedDate: 'Corrected visit date',
      correctNotes: 'Correct notes',
      correctedNotes: 'Corrected clinical notes',
      saveCorrection: 'Save correction',
      cancel: 'Cancel',
      voidRecord: 'Void record',
      voidReason: 'Reason for voiding',
      confirmVoid: 'Confirm void',
      voidedReason: 'Reason:',
      undoAttendance: 'Undo mistaken attendance',
      actionError: 'The visit record was not changed.',
    },
    vitalsAtScreening: 'Vital signs at screening',
    openReferral: 'Open the referral',
    recordDiagnosticResult: 'Record the diagnostic result in the referral',
    outcomeHint: 'Cured: bacteriologically confirmed pulmonary TB with a negative sputum result in the last month of treatment and on at least one earlier test. Treatment completed: treatment finished without that evidence. The system records your decision; it does not make it.',
    lab: {
      title: 'Laboratory results during treatment',
      add: 'Add lab result',
      schedule: 'DOH-NTP follow-up sputum for drug-susceptible pulmonary TB: end of month 2, month 5, and end of treatment. Each result is entered by staff as the laboratory reported it.',
      testType: 'Test',
      purposeLabel: 'Treatment point',
      resultDate: 'Result date',
      sampleId: 'Lab sample ID (optional)',
      resultLabel: 'Result',
      notes: 'Notes (optional)',
      save: 'Save lab result',
      saving: 'Saving…',
      empty: 'No laboratory results recorded for this case yet.',
      loadError: 'Could not load the laboratory results.',
      actionError: 'The laboratory result was not changed.',
      test: { xpert: 'Xpert MTB/RIF', smear: 'Sputum smear (DSSM)', culture: 'Culture', other: 'Other test' },
      purpose: { baseline: 'Baseline (diagnosis)', month_2: 'End of month 2', month_5: 'Month 5', end_of_treatment: 'End of treatment', other: 'Other' },
      outcome: { positive: 'Positive', negative: 'Negative', invalid: 'Invalid / no result' },
    },
    vitalsLog: {
      title: 'Vital signs during treatment',
      add: 'Record vitals',
      intro: 'Measurements taken at treatment visits. Record whatever was measured; any field may be left blank.',
      measuredOn: 'Measured on',
      save: 'Save vitals',
      saving: 'Saving…',
      empty: 'No vital signs recorded during this case yet.',
      loadError: 'Could not load the vital signs.',
      actionError: 'The vital signs were not changed.',
    },
    timeline: {
      title: 'Patient care timeline',
      privacy: 'A factual history from records visible to this facility. Laboratory result values, measurements, clinical free text, contact details, and voided records are excluded.',
      loadError: 'Could not load the patient timeline',
      empty: 'No timeline events are visible for this patient.',
      undatedTitle: 'Recorded, date unknown',
      undatedHelp: 'These historical states predate the referral audit trail. They are shown without an invented date.',
      dateUnknown: 'Date unknown',
      derived: 'scheduled date',
      system: 'System action',
      recordedByRole: 'Recorded by {{role}}',
      referralRecommended: 'Screening criteria indicated a referral.',
      noReferralRecommended: 'No referral was indicated by the screening criteria.',
      destination: 'Destination: {{code}}',
      statusChange: '{{from}} → {{to}}',
      labDetail: '{{test}} · {{purpose}}',
      scheduledFor: 'Scheduled for {{date}}',
      smsDetail: '{{kind}} — {{status}}',
      limit: 'Showing the earliest 500 events. Newer events may be omitted.',
      event: {
        patient_enrolled: 'Patient enrolled',
        screening_recorded: 'Screening recorded',
        referral_submitted: 'Referral submitted',
        referral_received: 'Referral received',
        lab_result_recorded: 'Laboratory result recorded',
        patient_did_not_present: 'Patient did not present',
        referral_closed: 'Referral closed',
        case_registered: 'TB case registered',
        treatment_started: 'Treatment started',
        appointment_scheduled: 'Appointment scheduled',
        sms_sent: 'SMS action recorded',
        appointment_attended: 'Appointment attended',
        appointment_overdue: 'Appointment overdue for review',
        appointment_missed: 'Appointment marked missed',
        appointment_cancelled: 'Appointment cancelled',
        followup_recorded: 'Treatment visit recorded',
        case_vitals_recorded: 'Vital signs recorded',
        case_lab_result_recorded: 'Laboratory result recorded (treatment)',
        case_status_changed: 'Case status changed',
        case_transferred: 'Case transferred',
        case_closed: 'Case closed',
        other: 'Care event',
      },
      messageKind: { reminder: 'Reminder', follow_up: 'Follow-up message' },
      delivery: {
        queued: 'Queued',
        // Task 6.4: the gateway returns `sent` the moment the provider accepts the
        // HTTP request. Nothing in the system knows whether a handset received
        // it, so the label must not say it was delivered. The stored value is
        // unchanged; only the wording is.
        sent: 'Accepted by provider (delivery unknown)',
        failed: 'Failed',
        stubbed: 'Test delivery',
      },
    },
  },
  detail: {
    caseSection: 'TB case',
    caseDecisionNote:
      'Create a case only after a clinician has enrolled this patient for treatment. The system does not infer that decision from the screening or result.',
    caseArrivalRequired: 'The patient must be received at this facility before a case can be created.',
    noCase: 'No TB case is linked to this referral.',
    createCase: 'Create TB case',
    creatingCase: 'Creating case…',
    caseCreateError: 'The case could not be created.',
    caseLookupError: 'Case status is unavailable. Refresh before creating a case.',
    openCase: 'Open case',
    registeredHere: 'Registered at this facility by {{name}}',
    sampleIdLabel: 'Laboratory sample ID',
    sampleIdPlaceholder: 'As written on the sputum cup',
    sampleIdSave: 'Save sample ID',
    sampleIdHint:
      'Assigned here, when the sputum sample is collected. The BHW app does not generate it.',
    sampleIdPending: 'Available once the patient arrives and the referral is marked received.',
    screeningSummary: 'Screening summary',
    actionsSection: 'Actions',
    patientReportedTag: 'patient-reported',
    screenedBy: 'Screened by {{name}} (BHW)',
    markReceived: 'Mark received',
    outcomePositive: 'Positive',
    outcomeNegative: 'Negative',
    resultLabel: 'Result notes (free text, optional)',
    resultPlaceholder: 'e.g. GeneXpert: MTB not detected',
    saveResult: 'Save result (marks as tested)',
    resultSavedOn: 'Result recorded {{date}}',
    markPresented: 'Presented',
    markNoShow: 'No-show',
    closeReferral: 'Close referral',
    appointmentsSection: 'Patient check-up appointments',
    appointmentsScope: 'Check-ups tied to this referral. Treatment visits appear in the TB case.',
    noAppointments: 'No appointments recorded for this referral.',
    apptScheduled: 'Scheduled',
    apptAttended: 'Attended',
    apptMissed: 'Missed',
    apptCancelled: 'Cancelled',
    markAttended: 'Mark attended',
    markMissed: 'Mark missed',
    cancelAppointment: 'Cancel appointment',
    recordAttendance: 'Record attendance',
    editAttendDate: 'Edit date',
    apptUndo: 'Undo',
    attendConfirm: 'Save attendance',
    attendDateLabel: 'Date the patient attended (today or earlier)',
    apptOnTime: 'on time',
    apptEarly: '{{count}}d early',
    apptLate: '{{count}}d late',
    relToday: 'today',
    relYesterday: 'yesterday',
    relTomorrow: 'tomorrow',
    relDaysAgo: '{{count}} days ago',
    relInDays: 'in {{count}} days',
    relMonthsAgo: '{{count}} months ago',
    relInMonths: 'in {{count}} months',
    relYearsAgo: '{{count}} years ago',
    relInYears: 'in {{count}} years',
    updateError: 'Update failed: {{message}}',
    notFound: 'Referral not found (or not addressed to your facility).',
    readOnly: 'Read-only',
    pgisLabel: 'Symptom severity (PGI-S)',
    pgisNote: 'Supplementary context only — not used to decide referral.',
    recvLabel: 'Referral receipt',
    recvUndo: 'Undo received',
    recvLocked: 'Locked after testing',
    labTitle: 'Laboratory outcome',
    labHint: 'Recorded by staff after testing — not computed by the system.',
    resultNone: 'Not yet recorded',
    attendLabel: 'Attendance',
    closedTag: 'Closed',
    footerNote:
      'Positive and negative outcomes are recorded by staff, never computed by the system.',
    selectTitle: 'Select a referral',
    selectBody:
      'Choose a patient from the inbox to view their screening and record outcomes.',
    reopen: 'Reopen referral',
    scheduleBtn: 'Schedule check-up',
    scheduleDateLabel: 'Check-up date',
    scheduleConfirm: 'Schedule',
    cancel: 'Cancel',
    scheduling: 'Scheduling…',
    scheduleError: 'Couldn’t save the check-up. Please try again.',
    dateFutureHint: 'Today or later',
    retry: 'Retry',
  },
  address: {
    region: 'Region',
    province: 'Province',
    city: 'City / Municipality',
    barangay: 'Barangay',
    sitio: 'Sitio / Purok',
  },
  symptoms: {
    cough_2wks: 'Cough for 2 weeks or more',
    weight_loss: 'Unexplained weight loss',
    night_sweats: 'Night sweats',
    fever: 'Unexplained fever',
    hemoptysis: 'Coughing up blood (hemoptysis)',
    chest_pain: 'Chest pain',
    fatigue: 'Fatigue / tiredness',
    loss_of_appetite: 'Loss of appetite',
    tb_contact: 'Close contact with a person known to have TB',
  },
  pgis: {
    none: 'None',
    mild: 'Mild',
    moderate: 'Moderate',
    severe: 'Severe',
  },
  register: {
    intro:
      'For a patient who came here directly, without a BHW referral. This records the same details and the same DOH-NTP checklist, and files the referral as already received.',
    optional: '(optional)',
    patientSection: 'Patient details',
    firstName: 'First name',
    middleName: 'Middle name',
    lastName: 'Last name',
    birthdate: 'Date of birth',
    age: 'Age',
    sex: 'Sex',
    consentSection: 'Consent',
    consentIntro:
      'Explain the purpose of this pre-screening to the patient in their own language before continuing.',
    consentConfirm: 'The patient consents to pre-screening and referral.',
    smsOptIn: 'Send SMS check-up reminders to this patient',
    contactNumber: 'Mobile number',
    smsLanguage: 'Language for reminder texts',
    contactInvalid: 'Enter a valid mobile number, or turn off SMS reminders.',
    checklistSection: 'DOH-NTP symptom checklist',
    checklistIntro:
      'Ask each question and record the patient\u2019s answer. These answers alone decide the referral recommendation.',
    pgisSection: 'Symptom severity (PGI-S)',
    pgisIntro:
      'How severe the patient says their cough is, in their own words. Supplementary context only.',
    answerAll: 'Answer every checklist item and the severity question to continue.',
    willFlag: 'These answers meet the presumptive TB criteria \u2014 the referral will be flagged.',
    willNotFlag:
      'These answers do not meet the presumptive TB criteria. The patient is still registered and the record is still kept.',
    saveCta: 'Register patient',
    saving: 'Registering\u2026',
    saveError: 'Could not register this patient: {{message}}',
    doneTitle: '{{name}} is registered',
    doneFlagged:
      'The screening meets the presumptive TB criteria. The referral is filed as received at this facility.',
    doneNotFlagged:
      'The screening does not meet the presumptive TB criteria. The record is filed at this facility so the visit is not lost.',
    recordLab: 'Record sample & lab result',
    nextStep: 'Next: record the lab sample ID and the laboratory result on this patient’s referral. A positive result then lets you open a TB case.',
    registerAnother: 'Register another patient',
    required: 'Required',
    lookupTitle: 'Search the shared patient registry first',
    lookupIntro:
      'Search by the patient’s exact name, birthdate, and optional mobile number before creating a record. Facilities across Bukidnon reuse one patient identity while clinical episodes remain access-controlled.',
    lookupAction: 'Search patient registry',
    lookupSearching: 'Searching…',
    lookupRequired: 'Complete the identity fields and search before registration can continue.',
    lookupClear: 'No matching patient was found. A new patient identity may be created.',
    lookupError: 'The registry search failed. Retry before registering this patient.',
    lookupFound: 'Possible existing patient found',
    noStoredPhone: 'No stored phone',
    useExisting: 'Use this existing patient',
    existingRestricted: 'Existing record found; coordinate with the patient’s barangay or receiving facility.',
    existingReused: 'The existing shared patient identity was reused; no duplicate patient was created.',
  },
  vitals: {
    heading: 'Vital signs',
    optionalTag: 'Optional',
    introFacility:
      'Record whatever was measured. Any field may be left blank, and none of it affects the referral recommendation.',
    contextNote:
      'Measurements only \u2014 supplementary context, not used to decide referral.',
    noneRecorded: 'No vital signs were recorded for this screening.',
    height: 'Height',
    weight: 'Weight',
    temperature: 'Temperature',
    systolic: 'Systolic BP',
    diastolic: 'Diastolic BP',
    pulse: 'Pulse rate',
    spo2: 'Oxygen saturation',
    bmi: 'BMI',
    bmiDerived: 'BMI (derived)',
    bloodPressure: 'Blood pressure',
    unitCm: 'cm',
    unitKg: 'kg',
    unitC: '\u00b0C',
    unitMmHg: 'mmHg',
    unitBpm: 'bpm',
    unitPercent: '%',
    unitBmi: 'kg/m\u00b2',
    outOfRange: 'Check the readings marked in red \u2014 those values look like typing slips.',
  },
};

// Full translation shape; tl/ceb must supply every key (compile error if not).
export type Translation = typeof en;

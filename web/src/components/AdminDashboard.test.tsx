 ...
 *
 * Before 0023 the Captains and BHWs tiles counted deactivated accounts inside
...
describe('AdminDashboard deactivated split', () => {
  it('shows no deactivated note when every account is active', async () => {
    mock.db.rows = [facility({ captains: 1, bhws: 5 })];
    ...
    expect(tileText('Captains')).not.toMatch(/deactivated/i);
  });
  ...
  it('sums both figures across facilities', async () => {
    ...
    mock.db.rows = [
      facility({ captains: 1, captains_inactive: 1, bhws: 2, bhws_inactive: 3 }),
      facility({ captains: 0, captains_inactive: 2, bhws: 3, bhws_inactive: 1 }),
    ];
    render(<AdminDashboard />);
    await waitFor(() => expect(screen.getAllByText('BHWs').length).toBeGreaterThan(0));
    ...
    expect(tileText('Captains')).toContain('1');
    expect(tileText('Captains')).toMatch(/3 deactivated/i);
  });

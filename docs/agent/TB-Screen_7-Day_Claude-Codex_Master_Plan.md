...
> **Gate status — 2026-09-10:** The seven-day capstone scope is complete. A user-requested post-plan extension is also implemented: migration 0039 adds an exact-match shared Bukidnon patient identity registry, idempotent facility reuse, and Barangay Report v2 case/outcome aggregates. The case detail now groups referred pre-screening/PGI-S, vitals, lab report, and appointments. Migration 0039 passed a 15/15 linked rollback matrix and is live. Full regression is 454/454 (portal 184, mobile 219, edge 51), the portal production build passes, mobile/portal TypeScript checks pass, and Expo Doctor is 21/21. This extension preserves the plan's privacy boundary: identity matching is province-wide, while clinical episodes remain facility-scoped. See `HANDOFF.md` and `CODEX_REVIEW.md`.
...
- [x] treatment status
- [x] appointment/case integration
- [x] follow-up recording
- [x] missed follow-up logic
- [x] timezone tests
- [x] Codex review passed

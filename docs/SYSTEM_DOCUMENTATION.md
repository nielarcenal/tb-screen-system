...
A scheduled Edge Function (`sms-reminders`) runs daily at 09:00 Asia/Manila (pg_cron at 01:00 UTC invoking the function via `pg_net`, authenticated by a shared `CRON_SECRET` header on top of the platform JWT check). It selects appointments scheduled for **tomorrow** whose patient opted into SMS, sends one reminder per appointment through a **swappable gateway** module, and logs every attempt to `sms_log` (idempotent — already-reminded appointments are skipped on re-runs).

- **Gateway abstraction:** `SMS_GATEWAY=stub` (current) logs sends without texting — the full pipeline is exercisable without SMS credits; switching to live sending is configuration only: `SMS_GATEWAY=semaphore` + `SEMAPHORE_API_KEY` (Semaphore is a Philippine SMS provider).
...
- Offline sign-out cannot push pending work (the dialog warns); SMS delivery status is stubbed until a live gateway key is configured.

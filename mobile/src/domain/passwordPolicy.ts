 ...
 * gate. Pure — no React, no React Native, no I/O — so it can be unit-tested
 * (see vitest.config.mts for why that constraint is not negotiable).
 *
 * Mirrored, deliberately, at web/src/lib/passwordPolicy.ts: the two packages
 * share no workspace, and the portal gate has to apply the same rules as the
 * app gate. Change one, change the other — the tests live in both trees.
 *
